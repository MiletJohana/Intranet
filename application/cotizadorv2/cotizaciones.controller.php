...
if (isset($_GET["id_coti"])) {
   require_once '../../resources/librerias/vendor/autoload.php';
   $cot = $_GET["id_coti"];
   ...
   <table align="center">
      <tr>
         <td width="100%"><img src="../../resources/img/documentos/fondoCot1.png" width="100%"></td>
      </tr>
   </table>
   ', 'O');
   ...
   <table align="center">
      <tr>
         <td width="100%"><img src="../../resources/img/documentos/imagen3.png" width="100%"></td>
      </tr>
   </table>
   ', 'O');
...
if (isset($_POST['action'])) {
   switch ($_POST['action']) {
         ...
         if (isset($_POST['verifi']) && $_POST['verifi'] == 1) {
            ...
            if (isset($_POST["productos"])) {
               ...
               for ($i = 0; $i < count($productos); $i++) {
                  $sqlP = "SELECT uni_emp_mq FROM productos WHERE id_prod=" . $productos[$i];
                  $queryP = $conexion->query($sqlP);
                  $rP = $queryP->fetch(PDO::FETCH_ASSOC);
                  $total = $total + ($precios[$i] * $cantidad[$i] * $rP['uni_emp_mq']);
               }
               ...
               $r = $query->fetch(PDO::FETCH_ASSOC);
               $cot = $r["max(id_coti)"] + 1;
               $ruta = "../cotizadorv2/cotizaciones.controller.php?id_coti=" . $cot . "&tip_cot=" . $tip_cot . "&id_cli=" . $id_cli . "&id=" . $id . "&id_cont=" . $id_cont;
      ...
      case 'emailMargen':
         $sqlC="SELECT * FROM cot_cotizaciones order by id_coti desc limit 1";
         $queryC=$conexion->query($sqlC);
         $rcot=$queryC->fetch(PDO::FETCH_ASSOC);
         $cot=$rcot['id_coti'];
         $sqlmarg="SELECT margen FROM cot_pro_x_cot WHERE (margen='18%' oR margen='16%') AND id_coti = $cot";
         $querymarg=$conexion->query($sqlmarg);
         $rM=$querymarg->fetch(PDO::FETCH_ASSOC);
         if(empty($rM['margen'])){
            $sqlUpcot="UPDATE cot_cotizaciones  SET id_estmarg = 0 ";
            $queryUpcot=$conexion->query($sqlUpcot);
         }else {
            $sqlUpcot="UPDATE cot_cotizaciones  SET id_estmarg = 1 WHERE id_coti = $cot";
            $queryUpcot=$conexion->query($sqlUpcot); 
         }
         $sqlCot= "SELECT id_estmarg FROM cot_cotizaciones WHERE id_coti= $cot";
         $queryCot=$conexion->query($sqlCot);
         $rC=$queryCot->fetch(PDO::FETCH_ASSOC);
         $est=$rC['id_estmarg'];

         if ($est == 1){
            include 'EmailMargen.php';
         }  
      ...
      case 'updateCot':
         ...
         if (isset($_POST['verifi']) && $_POST['verifi'] == 1) {
            ...
            if (isset($_POST["productos"])) {
               ...
               $cot = $r["id_coti"];
               $id = $r["id_usu"];
               $ruta = "../cotizador/cotizaciones.controller.php?id_coti=" . $cot . "&tip_cot=" . $tip_cot . "&id_cli=" . $id_cli . "&id=" . $id . "&id_cont=" . $id_cont;
      ...
      case 'verifyProd':
         ...
         $sqlP = "SELECT * FROM `cot_productos` WHERE cod_ref='" . $_POST['cod_pro'] . "'";
         $queryP = $conexion->query($sqlP);
      ...
      case 'remCiu':
         ...
         if ($query != null) {
            echo '<select class="form-control" id="remCiu" name="remCiu" style="height: 35px;" required>
         ...
         case 'email':
            ...
            case 'comentario':
               $cot=$_POST['id_coti'];
               $sqlRech=" INSERT INTO cot_descrip_rech(id_coti, 
                                                      desc_rechazo)
                                                VALUES('$cot',
                                                       '".$_POST['desc_rec']."')";
               $queryRech=$conexion->query($sqlRech);

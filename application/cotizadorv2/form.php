...
if (isset($_POST['edit']) && $_POST['edit'] != '' && ($_POST['resp'] == 6 || $_POST['resp'] == 11)) {
	...
	$sqlMargE="SELECT * FROM cot_margen";
	$queryMargE=$conexion->query($sqlMargE);
	}else if($_POST['est_vigen']==0){
		$sql1 = "SELECT * FROM cot_cotizaciones AS cot, cot_tip_cotizacion AS tip, mq_clientes AS cli, contactos AS con
			WHERE cot.id_tip_cot=tip.id_tip_cot
			AND cot.id_cli=cli.id_cli
			AND cot.id_cont=con.id_cont
...
if ($_POST['resp'] == 18) {
	...
	$sqlMarg="SELECT * FROM cot_margen";
	$queryMarg=$conexion->query($sqlMarg);
...
<form role="form" id="form-cotizaciones">
	...
	<div class="row">
		<div class="col-12 col-sm-12 mt-5" align="center">
			<h3 align="center">Datos Productos </h3>
			<hr class="mx-auto" style="width:60%;">
		</div>
	</div>
	<div class="alert alert-danger alert-dismissible fade show mt-2" id="marg_alert" name="marg_alert" role="alert" style="display:none;" >
	   Recuerde que los productos con margen de <strong>16%</strong> y <strong>18 %</strong>necesitan aprobación. Por esta razón la cotización quedara en estado
	   <strong><i>Pendiente por aprobación</i></strong>
	...
	<div class="row">
		...
		<div class="col-md-3 col-sm-12 mb-3">
			<input type="text" name="cod_ref" id="cod_ref" class="form-control" placeholder="Codigo del producto" readonly>
			<input type="hidden" name="cod_pro" id="cod_pro">
		</div>
		<div class="col-md-2 col-sm-12 mb-3">
			<input type="text" name="und_emp" id="und_emp" class="form-control" placeholder="Unidad de medida" readonly>
		</div>
		<div class="col-md-1 col-sm-12 mb-3">
		<input type="number" name="can_emp" id="can_emp"  class="form-control" placeholder="U/E MQ" readonly>
		</div>
	</div>
	<div class="row">
		<div class="col-md-12 col-sm-12 mb-3">
			<textarea name="des_pro" id="des_pro" class="form-control" placeholder="Descripción" disabled style="height: 73px;"></textarea>
		</div>
	</div>
	<div class="row">
		<div class="col-md-3 col-sm-12 mb-3">
		...
		<div class="col-md-3 col-sm-12 mb-3">
			<select class="form-select" id="marg_prod" onchange="calcuPreci(this.value);">
			<?php if (!isset($_POST['edit'])) { ?>
                <option value="0">Margen</option>
            <?php } while ($rM =$queryMarg->fetch(PDO::FETCH_ASSOC)){?>
                <option value="<?php echo $rM['id_margen']; ?>" class="btn bg<?php echo $rM['color'];?> dropdown-item"><?php echo $rM['nom_margen'];?> </option>
            <?php }
            if(($_POST['resp'] == 6) || ($_POST['resp']==11)){
                while ($rMedit = $queryMargE->fetch(PDO::FETCH_ASSOC)){?>
                    <option value="<?php echo $rMedit['id_margen']; ?>" <?php echo $rMedit['nom_margen']; ?></option>
            <?php }} ?>
			    
			</select>
		</div>
		<div class="col-md-6 col-sm-12 mb-3">
			<input type="text" name="pre_pro" id="pre_pro" class="form-control" placeholder="Precio" disabled>
			<input type="hidden" name="pre_base" id="pre_base">
			<input type="hidden" name="est_mar" id="est_mar"> 
			<input type="hidden" name="porcet" id="porcet"> 
				...
				<div class="tab-final">
					<table id="table" class="table" style=" font-size: 100%;">
						<tr style="background-color: #363a41; color: white;">
							...
							<td width="10%">**Obser</td>
							<td width="10%">U.M</td>
							<td width="10%">U/E</td>
							<td width="10%">Precio U.E</td>
							<td width="10%">Cant</td>
							<td width="10%">Margen</td>
							<td width="20%">Total</td>
						</tr>
						<?php
						if (isset($_POST['edit']) && ($_POST['resp'] == 6 || $_POST['resp'] == 11)) {
							$sql3 = "SELECT * FROM `cot_pro_x_cot` pxc,`cot_cotizaciones` cot,`productos` pro
												WHERE pxc.id_coti=cot.id_coti
												AND pxc.cod_pro=pro.id_prod
												AND cot.id_coti=" . $_POST['edit'];
							$query3 = $conexion->query($sql3);
							while ($r = $query3->fetch(PDO::FETCH_ASSOC)) { ?>
								<tr id="<?php echo $r['cod_pro']; ?>">
									...
									<td><?php echo $r['id_uni_med']; ?></td>
									<td><?php echo $r['uni_emp_mq']; ?></td>
									<td><?php echo $r['pre_cot']; ?></td>
									<td><?php echo $r['can_com']; ?></td>
										<?php if ($r['margen'] == NULL){?> 
											<td>---</td>
										<?php } else {?>
...
<script type="text/javascript" src="../js/prodV2.js"></script>
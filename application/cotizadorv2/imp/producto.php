...
while ($r = $query4->fetch(PDO::FETCH_ASSOC)) {
	...
	//Terminado Aqui
	$html .= '<tr>
				<td style="text-align:center; background-color:transparent;"><div><img style="max-width:100px;" src="'.$r["url_img"].'"></div><p>' . $r["cod_pro"] . '</p></td>
				<td style="background-color:transparent;"><strong>' . $r["nom_pro_cot"] . '</strong><br><p>' . $r["des_pro_cot"];
	'</p><br>';
	if ($r['obs_prod'] != '') {
		$html .= '<br><br><p>**' . $r["obs_prod"] . '</p>';
	}
	$html .= '<br><p><strong>UNIDAD DE EMPAQUE:</strong> ' . $r["id_uni_med"] . ' ' . $r["uni_emp_mq"] . '</p>';
...
<table style="align-content: left; line-height:1.5; font-size:14px;">
 ...
 <table style="font-size:14px;font-style:Arial;background-color:transparent;">
...
<table style="font-size:15px;font-style:Arial;background-color:transparent;">
 	<tr>
 		<td style="text-align:left;background-color:transparent;">
 			<p style="letter-spacing:0.1px;">
				Solicitamos enviar sus órdenes de compra al correo electrónico <a href="mailto:' . $cotizanteCoti["eml_usu"] . '">' . $cotizanteCoti["eml_usu"] . '</a> nos permite ser más ágiles en la grabación del pedido y ejercer  controles inherentes al proceso.
			</p>
 ...
 <div>
<p style="color:black;font-size:14px;font-style:Arial;">CONDICIONES COMERCIALES :</p>
<table style="align-content: left; line-height:1.5; font-size:14px; background-color:transparent;">
	<tr>
		<td style="background-color:transparent;">
			Tiempo de entrega:  <br> 
			Forma de pago:<br>

...
function autoProd(param) {
    if (param == 1) {
        $("#nom_pro").autocomplete({
            source: '../autocomplete/cotizador/productos/buscador.php',
            minLength: 3,
            select: function (event, ui) {
                $("#des_pro").val(ui.item.des_pro);
                $("#und_emp").val(ui.item.und_emp);
                $("#can_emp").val(ui.item.can_emp);
...
function autoCont() {
    $("#nom_cont3").autocomplete({
        source: '../autocomplete/cotizador/contactos/buscador.php',
        minLength: 3,
        select: function (event, ui) {
...
function crearCotizacion(param) {
    if (validarCampos('form-cotizaciones') == 0) {
        Swal.fire({
            title: "Estas seguro de crear Cotización",
            icon: "info",
            ...
            if (result.isConfirmed) {
                ...
                if (
                    ...
                    if (childrens.length <= 1) {
                        alertWarning("Necesitas tener una lista de productos");
                    } else {
                                ...
                                if (resp == 1) {
                                    $.ajax({
                                        ...
                                        success: function (resp) {
                                            ...
                                            if (param == 2) {
                                                $('#modal-footer-sm').html('<button type="button" class="btn btn-secondary" onclick="refreshTable(18, ' + id_cli + ')" data-bs-dismiss="modal">Cerrar</button>');
                                            } else {
                                                $('#modal-footer-sm').html('<button type="button" class="btn btn-secondary" onclick="refreshTable(1, 0)" data-bs-dismiss="modal">Cerrar</button>');   
                                            }
                    ...
                    alertWarning('Debes escoger el Asesor o Representate responsable');
                }
            }
          });
    }
                            ...
                            if (resp == 1) {
                                $.ajax({
                                    ...
                                    success: function (resp) {
                                        ...
                                        $('#smallModal').modal('show');
                                        $('#btn-footer-sm').html('<button type="button" class="btn btn-secondary" onclick="refreshTable(1)" data-bs-dismiss="modal">Cerrar</button>');
                                        //alert("Cotización creada");
                                    }
                                });

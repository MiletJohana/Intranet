    ...
    if (validarCampos(param) == 0) {
        ...
        $.ajax({
            ...
            success: function(resp) {
                if(resp == 3){
                    toastSuccess('Cargo Creado Correctamente');
...
function form(params) {
    if (params == 1) {
        ...
        if(mes != ''){
            ...
            alertWarning('Por favor seleccione una fecha');
            //$('#mes').trigger('focus');
        }
        ...
        if(year != ''){
            $('#formExcelAnual').append('<input type="hidden" name="year" id="year" value="' + year + '">');
...
function modInd(param, carpeta, table) {
    if (validarCampos(param) == 0) {
        ...
        $.ajax({
            ...
            success: function(resp) {
                $('#largeModal').modal('hide');
                console.log(param,carpeta,table);
    ...
    $.ajax({
        ...
        success: function(resp) {
            ...
            if (resp == 1) {
                ...
                $('#respPer').html('<br><br><div class="alert alert-success alert-dismissible fade show mt-4 d-flex align-items-center" role="alert"><i class="fa-solid fa-circle-check me-3 fa-xl"></i>Seguimiento ' + id_solC + ' actualizado correctamente <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>');
            } else {
                $('#resp').html(resp);
                $('#respPer').html('<br><br><div class="alert alert-danger alert-dismissible fade show mt-4 d-flex align-items-center" style="font-size:15px;" role="alert"><i class="fa-solid fa-circle-exclamation me-3 fa-xl"></i>No se ha podido modificar ' + resp + '</div>');
            }
        }
    });
}
    ...
    $.ajax({
        ...
        success: function(resp) {
            if (resp == 1) {
                ...
                $('#resp').html('<div class="alert alert-success alert-dismissible fade show mt-4 d-flex align-items-center" role="alert" style="margin-top:15px;"><i class="fa-solid fa-circle-check me-3 fa-xl"></i> Pago actualizado correctamente. <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>');
            } else {
                $('#resp').html(resp);
                $('#resp').html('<div class="alert alert-danger alert-dismissible fade show mt-4 d-flex align-items-center" style="font-size:15px; margin-top:15px;" role="alert"><i class="fa-solid fa-circle-exclamation me-3 fa-xl"></i>No se ha podido modificar' + resp + '</div>');
            }
        }
    });
}
        ...
        success: function(respHTML) {
            if (respHTML == 1) {
                if (param == 1) {
                    ...
                    toastSuccess('Observación agregada correctamente');
                }
            }
...
function filtro(params, table, title, id) {
     let tablu= $('#' +id ).DataTable();
     tablu.ajax.reload();
}

function filtroMes(params, table, title, id) {
...
function cumplirAct(id) {
    $.ajax({
        ...
        success: function(resp) {
            refreshTableInd('eventos', 'tabla.php');
            if (resp == 1) {
               alertSuccess('Actividad #' + id + ' actualizada correctamente', '',1 )
            } else {
                alertError('La actividad #' + id + ' no fue actualizada correctamente.')
            }
        }
    });
}
...
function cambiarEstado(id, est, param) {
    if (param == 1) {
        $.ajax({
            ...
            success: function(resp) {
                alertSuccess('Descuento ' + id + ' actualizado correctamente', '../descuentos/index.php?table=1', 2);
                $("#pass").attr("disabled", "");
                $("#apr").attr("disabled", "");
             }
        });
        ...
        $.ajax({
            ...
            success: function() {
                ...
                alertSuccess('Descuento ' + id + ' actualizado correctamente', '', 1);
            }
        });
    }
}
...
function habilitarOtro(val, param) {
    if (param == 0) {
        if (val == 4) {
            $("#div_otro_tip").removeClass("none");
            $("#otro_tip").removeAttr("readonly");
            $("#otro_tip").focus();
        } else {
            $("#div_otro_tip").addClass("none");
            $("#otro_tip").attr("value", "");
                ...
                if (respht == 1) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            $('#modal-body-sm').html(data);
                            $('#smallModal').modal('show');
                            $('#modal-footer-sm').html('<button type="button" class="btn btn-secondary" onclick="refreshTableInd(' + "'" + file + "'" + ',' + "'" + table + "'" + ')" data-bs-dismiss="modal">Cerrar</button>');
                        }
                    })
                } else {
                    $('#largeModal').modal('hide');
...
function ver(id, title, file) {
    ...
    $.ajax({
        ...
        success: function(respHtml) {
            $('#modal-body-lg').html(respHtml);
            cerrar = "<button type='button' class='btn btn-secondary' data-bs-dismiss='modal'>Cerrar</button>"

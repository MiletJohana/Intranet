    ...
    if (certCon != "" && copRu != "" && refBan != "" && form_cre != "") {
        if ($("#form-crm input[name='tipCli']:radio").is(':checked') &&
            ...
            if (validarCampos('form-crm') == 0) {
                $.ajax({
                    url: '../credito/credito.controller.php',
                    data: formuIngComer,
                    type: 'POST',
                    contentType: false,
                    processData: false,
                    success: function(resp) {
                        ...
                        $('#modal-footer-sm').html('<a href="../credito/index.php" class="btn btn-secondary">Cerrar</a>');
                    }
                });
            }
        } else {
...
function modificarCrm() {
    ...
    $.ajax({
        ...
        success: function(resp) {
            ...
            $('#modal-body-sm').html(resp);
            $('#smallModal').modal('show');
            $('#btn-footer-sm').html('<button type="button" class="btn btn-secondary" onclick="refreshTableCrm()" data-bs-dismiss="modal">Cerrar</button>');
        }
    });
}
...
function confirmEliminarcrm(id_sol) {
    ...
    $.ajax({
        ...
        success: function(resp) {
            ...
            $('#smallModal').modal('show');
            $('#modal-footer-sm').html('<a class="btn btn-secondary" href="../credito/index.php" data-bs-dismiss="modal">Cerrar</a>');;
            $.ajax({
                url: '../credito/boton.php',
                type: 'POST',
                data: { resp: 4 },
    ...
    if (validarCampos('observa') == 0) {
        $.ajax({
            ...
            success: function(resp) {
                ...
                $('#smallModal').modal('show');
                $('#btn-footer-sm').html('<button type="button" class="btn btn-secondary" onclick="refreshTableCrm()" data-bs-dismiss="modal">Cerrar</button>');
                $('#btn-footer-sm').html('<a href="../credito/index.php" class="btn btn-secondary">Cerrar</a>');
                if (param == 3 || param == 4) {
                    if (param == 3) {
                        var resp1 = { resp: 8 };
                    } else if (param == 4) {
...
function desp2() {
    $.ajax({
        ...
        success: function(resp) {
            ...
            $('#hora').attr('id', 'hor2_2');
            //$('#btnMen11').html('<div class="col-md-1 col-sm-12 d-flex"><button id="btnMen1" type="button" class="btn btn-sm btn-danger" onclick="remove(1);"><i class="fa fa-remove"></i></button></div>');
        }
    });
}
...
function auto() {
    $("#nom_clie").autocomplete({
        source: '../autocomplete/credito/buscador.php',
        minLength: 3,
        select: function(event, ui) {
            $("#id_nit").val(ui.item.id_cli);
            ...
            $('#car').val(ui.item.cargo_conta);
            $('#TelF').val(ui.item.telefono_fijo);
            $('#prod').val(ui.item.prod_preg);
            $('#Ext').val(ui.item.ext_tel);
            if (ui.item.ase_com != '0') {
                $("#aseCom").val(ui.item.ase_com);
                $("#nomAse").val(ui.item.nom_usu);
                $("#aseCom").attr('readonly', '');
            }
            $("#id_nit").attr('readonly', '');

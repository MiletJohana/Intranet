...
function agregarEnrutamiento() {
    ...
    $.ajax({
        ...
        success: function(resp) {
            ...
            $('#modal-footer-sm').html('<button type="button" class="btn btn-secondary" onclick="refreshTableEnrut()" data-bs-dismiss="modal">Cerrar</button>');
        }
    });
}
...
function imrpimirEnrutamiento(num_enr) {
    ...
    $.ajax({
        ...
        success: function(resp) {
            $('#modal-body-lg').html(resp);
            $('#info').html('<div class="alert alert-danger alert-dismissible fade show mt-4 d-flex align-items-center" style="font-size:15px;" role="alert"><i class="fa-solid fa-circle-exclamation me-3 fa-xl"></i>¿Está seguro que desea imprimir la ruta ' + num_enr + '? <br> Recuerde que de confirmar no se podra modificar ninguna de las diligencias ni se podra eliminar la ruta.</div>');
    ...
    $.ajax({
        ...
        success: function(resp) {
            if (resp = "1") {
                ...
                $('#modal-body-sm').html("Enrutamiento # '" + num_enr + "'<br>Estado: EN RUTA");
                $('#modal-title-sm').html('Aviso');
                $('#smallModal').modal('show');
                $('#modal-footer-sm').html('<button type="button" class="btn btn-secondary" onclick="refreshTableEnrut()" data-bs-dismiss="modal">Cerrar</button>');
            } else {
...
function eliminarEnrut(id) {
    $('#modal-title-md').html('Eliminar Enrutamiento '+id);
    $('#modal-body-md').html('¿Está seguro de eliminar este Enrutamiento?');
    ...
    $.ajax({
        ...
        success: function(resp) {
            ...
            $('#btnEliminar').attr('onclick', 'confirmEliminar('+id+');');
        }
    });
}
...
function confirmEliminar(id) {
    ...
    $.ajax({
        ...
        success: function(resp) {
            $('#modal-title-sm').html('Enrutamiento '+id);
            $('#modal-body-sm').html(resp);
            $('#smallModal').modal('show');
            $('#btn-footer-sm').html('<button type="button" class="btn btn-secondary" onclick="refreshTableEnrut()" data-bs-dismiss="modal">Cerrar</button>');
            refreshTableEnrut();
        }
    });
}
...
function moverDiligencia(id) {
    $('#' + id).appendTo("#ruta");
    $("#tableDiligenciaEnr td:nth-child(4)").css("display", "none");
    $('#' + id + ' :nth-child(7)').html('<button type="button" id="btn-close-dil" onclick="retornarDiligencia(' + id + ')" class="btn btn-danger">' +
        '<i class="fa-solid fa-xmark"></i></button>');
        
}

function retornarDiligencia(id) {
    $("td:nth-child(4)").css("display", "");
    $('#' + id).appendTo("#diligencias");
    $('#' + id + ' :nth-child(7)').html('<button type="button" id="btn-check-dil" onclick="moverDiligencia(' + id + ')" class="btn btn-success">' +

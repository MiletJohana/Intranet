...
function eliminar(resp, elim, nom, conf) {
    if (conf == 1) {
        ...
        $.ajax({
            url: '../cotizador/cotizaciones.controller.php',
            type: 'POST',
            ...
            success: function(respHtml) {
                ...
                $('#modal-footer-sm').html('<button id="elim" type="button" class="btn btn-default" onclick="refreshTable(' + resp + ')" data-bs-dismiss="modal">Cerrar</button>');
            }
        });
    }
}
    ...
    if (validarCampos('form-SubCoti') == 0) {
        ...
        $.ajax({
            ...
            success: function(respHtml) {
                ...
                $('#modal-body-sm').html(respHtml);
                $('#smallModal').modal('show');
                $('#modal-footer-sm').html('<button type="button" class="btn btn-default" onclick="refreshTable(1)" data-bs-dismiss="modal">Cerrar</button>');
            }
        });
    }
}
...
function enviarCom(id_coti, id_cont, id_asc, id_mq, id_usu) {
    ...
    $.ajax({
        ...
        success: function(resp) {
            $('#mediumModal').modal('hide');
            $('#modal-title-sm').html('pruebas ');
            $('#modal-body-sm').html(resp);
            $('#smallModal').modal('show');
            if (id_mq == 1) {
                $('#modal-footer-sm').html('<a href="https://intranet.masterquimica.com/application/cotiAuto/index.php?id_coti=' + id_coti + '&xont=' + id_cont + '&asc=' + id_asc + '&ux=' + id_usu + '" class="btn btn-default">Cerrar</a>');
            } else {
                $('#modal-footer-sm').html('<a href="https://intranet.masterquimica.com/application/cotiAuto/index.php?id_coti=' + id_coti + '&xont=' + id_cont + '&asc=' + id_asc + '" class="btn btn-default">Cerrar</a>');
            }
        }
    });
}
...
function buscarClie(param) {
    if (param == 1) {
        ...
        $('#id_cli1').css("border", "");
        $('#natura').html('<div class="col-md-6"><label for="eml_cont3" class="control-label">Correo</label><input type = "email" class= "form-control" id = "eml_cli1" name = "eml_cli1" placeholder = "Correo electrónico" required ><input type="hidden" id="natura1" name="natura1" value="1"></div>');
    } else if (param == 2) {
        $('#id_cli1').css("border", "");
...
//     if (validarCampos('form-Contacto') == 0) {
...
// function buscarCont(param) {
//     if (param == 1) {
//         $('#nom_cont3').attr('type', 'text');
//         $('#buscarCont').html('Nuevo');
//         $('#buscarCont').attr('onclick', 'buscarCont(2)');
//         $('#btnContacto').html('Actualizar Contacto');
//         $('#btnContacto').attr('onclick', 'actualizarCont();');
//         $('#btnContacto').attr('disabled', '');
//         $('#accionCont').attr('value', 'updateCont');
//     } else {
//         $('#nom_cont3').attr('type', 'hidden');
//         $('#buscarCont').html('Editar Existente');
//         $('#buscarCont').attr('onclick', 'buscarCont(1)');
//         $('#btnContacto').html('Crear Contacto');
//         $('#btnContacto').attr('disabled', '');
//         $('#btnContacto').attr('onclick', 'crearContacto();');
//         $('#accionCont').attr('value', 'addCont');
//         $('#nom_cont3').val('');
//         $('#id_cli2').val('');
//         $('#nom_cli2').val('');
//         $('#nom_cont2').val('');
...
/// Funcioes del Cotizador para actualizar tabla ////
...
function actualizarTablaEstadisticas(id) {
    ...
    $.ajax({
        url: '../cotizador/table_stadistics.php',
...
function actualizarEstadoCotizacion(state, opc) {
    ...
    switch (opc) {
        case 1:
            a.attr("class", "btn btn-secondary btn-warning btn-sm bmd-btn-icon");
            break;
        case 2:
            a.attr("class", "btn btn-secondary btn-success btn-sm bmd-btn-icon");
            break;
        case 3:
            a.attr("class", "btn btn-secondary btn-danger btn-sm bmd-btn-icon");
            break;
        case 4:
            a.attr("class", "btn btn-secondary btn-info btn-sm bmd-btn-icon");
            break;
    }
}
...
function ctrlUsu() {
    ...
    $.ajax({
        url: '../cotizador/table_control.php',
        type: 'POST',
...
function ctrlResum() {
    ...
    $.ajax({
        url: '../cotizador/tablacontrol.php',
        type: 'POST',
        data: { id, id_usu: id, mes, mes2 },
        success: function(resp) {
...
function form(params) {
    if (params == 1) {
        ...
        var mes = $('#mes').val();
        $('#formExcel').append('<input type="hidden" name="id_usu" id="id_usu" value="' + id_usu + '"> <input type="hidden" name="mes" id="mes" value="' + mes + '">');
        $('#formExcel').submit();
    } else if (params == 2) {
        var id_usu = $('#id_usu').val();
        var mes = $('#mes').val();
        ...
        var id_usu2 = $('#id_usu2').val();
        var mes = $('#mesR').val();
        var mes2 = $('#mesR2').val();
        $('#formControl').append('<input type="hidden" name="id_usu2" id="id_usu2" value="' + id_usu2 + '"> <input type="hidden" name="mesR" id="mesR" value="' + mes + '"> <input type="hidden" name="mesR2" id="mesR2" value="' + mes2 + '">');
        $('#formControl').submit();
    }
}

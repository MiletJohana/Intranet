        ...
        $.ajax({
            ...
            success: function(resp) {
                if (resp.substr(0, 1) == 1) {
                    ...
                    if (id_cli == 0) {
                        $('#modal-footer-sm').html('<button type="button" class="btn btn-default" onclick="refreshTableCor(1, 0)" data-bs-dismiss="modal">Cerrar</button>');
                    } else {
                        $('#modal-footer-sm').html('<button type="button" class="btn btn-default" onclick="refreshTableCor(5, ' + id_cli + ')" data-bs-dismiss="modal">Cerrar</button>');
                    }
                } else {
                    $('#largeModal').modal('hide');
...
function crear(value) {
    if (value == 1) {
        $('#buton_prov').attr("class", "btn btn-danger ml-auto");
        ...
        $('#buton_prov').attr("class", "btn btn-primary ml-auto");
...
function auto2(param) {
    $("#search").autocomplete({
        ...
        select: function(event, ui) {
            ...
            cell7.innerHTML = '<a id="but_x" name="but_x" class="btn btn-danger btn-sm" required onclick="eliminarFac(' + ui.item.id_cli + param + ');"><i class="fa fa-close text-danger"></i></a>';
...
function verCorr(id, id_estSeg) {
    $('#modal-title-lg').html('Información');
    $.ajax({
        url: '../correspondencia2/infoCaja.php',
        data: { resp: 12, id_seg: id, id_estSeg: id_estSeg },
        type: 'POST',
        success: function(resp) {
            $('#modal-body-lg').html(resp);
        }
...
function editarCorr(id, id_estSeg, id_nom, id_prove) {
    ...
    $.ajax({
        url: '../correspondencia2/form.php',
        type: 'POST',
        data: { resp: 6, id_seg: id, id_estSeg: id_estSeg, value: id_estSeg, id_nom: id_nom, id_prove: id_prove },
        success: function(resp) {
            $('#modal-body-lg').html(resp);
            $('#largeModal-dialog').attr('style', 'min-width:1200px');
...
function actualiCorr() {
    ...
    $.ajax({
        ...
        success: function(resp) {
            ...
            $('#modal-body-sm').html(resp);
            $('#smallModal').modal('show');
            $('#btn-footer-sm').html('<button type="button" class="btn btn-default" onclick="refreshTableCor(4)" data-bs-dismiss="modal">Cerrar</button>');
        }
    })
}
    ...
    $.ajax({
        ...
        success: function(resp) {
            if (resp == 1) {
                ...
                $('#resp').html('<div class="col-md-12 alert alert-success"> <a href="#" class="close" data-bs-dismiss="alert" aria-label="close">×</a> Correspondencia ' + id_seg + ' actualizado correctamente </a> </div>');
                snackbar('Correspondencia ' + id_seg + ' actualizada correctamente <a class="btn btn-info" data-bs-toggle="modal" data-bs-target="#mediumModal" onclick="mostrarCorr(' + id + ', ' + id_estSeg + ');">Ver</a>', "snackbar", 0, true);
            } else {
                $('#resp').html(resp);
                snackbar('No se ha podido modificar', "snackbar", 0, true);
            }
        }
    });
}
    ...
    $.ajax({
        ...
        success: function(resp) {
            if (resp == 1) {
                ...
                $('#resp-text').html('Correspondencia ' + id_seg + ' finalizada correctamente');
                $('#resp').addClass('alert-success');
                $('#resp').removeAttr('style');
            } else {
                $('#resp').html(resp);
                $('#resp-text').html('No se ha podido modificar: '.resp);
...
function refreshTableCor(param, id_cli) {
    if (param == 5) {
        ...
        if (param == 1) {
            ...
            url = '../correspondencia2/tabla3.php';
        } else if (param == 4) {
...
function contaCorr(id_seg, id_nom, id_prove) {
    ...
    if (r == true) {
        $.ajax({
            ...
            success: function(resp) {
                $('#smallModal').modal('show');
                $('#modal-footer-sm').html('<button type="button" class="btn btn-default" onclick="refreshTableCor(2, 0)" data-bs-dismiss="modal">Cerrar</button>');
                $('#largeModal').modal('hide');
                $('#modal-title-sm').html('Seguimiento Actualizado');
                $('#modal-body-sm').html(resp);
...
function eliminarCorr(id_seg) {
    var r = confirm("¿Está seguro de Eliminar ésta Correspondencia?");
    if (r == true) {
        $.ajax({
            url: '../correspondencia2/correspondencia.controller.php',
            type: 'POST',
            ...
            success: function(resp) {
                ...
                $('#modal-body-sm').html(resp);
                $('#smallModal').modal('show');
                $('#modal-footer-sm').html('<button type="button" class="btn btn-default" onclick="refreshTableCor(1)" data-bs-dismiss="modal">Cerrar</button>');
            }
        });
    }
}
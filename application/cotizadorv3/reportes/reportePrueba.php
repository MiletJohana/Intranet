...
if ($query->rowCount() > 0) {
    ...
    /** Se agrega la libreria Spreadsheet */
    ...
    $StringAnio = ucwords(strftime("%Y", strtotime($fech)));
    $objPHPExcel->setActiveSheetIndex(0)
    ...
    while ($r = $query-> fetch(PDO::FETCH_ASSOC)) {
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A' . ($i) . ':B' . ($i))->applyFromArray($estiloInformacion);
        $objPHPExcel->getActiveSheet()->getStyle('D' . ($i) . ':S' . ($i))->applyFromArray($estiloInformacion);
        $i++;
    }
    ...
    $StringMes2 = ucwords(strftime("%b", strtotime($fech)));
    ...
    header('Content-Disposition: attachment;filename="Control-' . date("d") . '-' . $StringMes . '-' . date("Y") . $r2["nom_cns"] . '.xlsx"');

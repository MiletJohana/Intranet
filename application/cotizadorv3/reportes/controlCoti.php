...
if ($query->rowCount() > 0) {
    ...
    $objPHPExcel->getProperties()->setCreator("MaterQuimica") //Autor
        ->setLastModifiedBy("MaterQuimica") //Ultimo usuario que lo modificó
    ...
    // Se manda el archivo al navegador web, con el nombre que se indica
    header("Content-Type: text/html;charset=utf-8");
    header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    header('Content-Disposition: attachment;filename="Resumen-' . date("d") . '-' . $StrMonth . '/' . $StrMes2 . '-' . $anio . '.xlsx"');

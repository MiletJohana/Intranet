...
<div class="col-12 py-3">
    ...
    <div class="table-responsive-sm">
        <ul class="nav nav-underline ms-2 flex-nowrap">
            ...
            <?php if ($_SESSION['id'] == 1032472569  || $_SESSION['id'] == 1071628649) { ?>
            <li class="nav-item">
            ...
            <?php } ?>
            <?php /*if ($_SESSION['id'] == 1072448076 || $_SESSION['id'] == 1018437049  || $_SESSION['id'] == 1071628649) { ?>
            <li class="nav-item">
...
<div class="col-12">
    ...
    <button onclick="newCotizacion(1,<?php echo $_SESSION['id']; ?>);" class="btn btn-danger" data-bs-toggle="modal"
        data-bs-target="#largeModal">
        Crear Cotización
    </button>
    <button onclick="newCotizacion(15,<?php echo $_SESSION['id']; ?>);" class="btn btn-primary" data-bs-toggle="modal"
        data-bs-target="#largeModal">
        Subir Cotización
    </button>
    <a id="Opt"
    ...
    <div class="row justify-content-center mt-4">
        <div class="col-md-2 col-sm-12">
            ...
            <select class="form-select" name="id_usu" id="id_usu"
                onchange="cotUsu(<?php echo date('m') . ',' . date('Y') ?>);">
                <option value="<?php echo $_SESSION['id']; ?>">Mis cotizaciones</option>
                <?php
                $sql = "SELECT nom_usu, id_usu FROM mq_usu WHERE (id_are != 10) AND id_are IN (0,4,6,7,11,12) AND usu_elim = 0 AND id_usu != '" . $_SESSION['id'] . "'";
                $query = $conexion->query($sql);
                $sqlAng = "SELECT nom_usu, id_usu FROM mq_usu WHERE (id_are != 10) AND id_are IN (4,7) AND id_reg = 1 AND usu_elim = 0 AND id_usu != '" . $_SESSION['id'] . "'";;
                $queryAng = $conexion->query($sqlAng);
                $sqlcali= "SELECT nom_usu, id_usu FROM mq_usu WHERE (id_are != 10) AND id_are IN (0,4,6,7,11,12) AND id_reg = 2 AND usu_elim = 0 AND id_usu != '" . $_SESSION['id'] . "'";
                $querycali =$conexion->query($sqlcali);
                $sqlmedell= "SELECT nom_usu, id_usu FROM mq_usu WHERE (id_are != 10) AND id_are IN (0,4,6,7,11,12) AND id_reg = 3 AND usu_elim = 0 AND id_usu != '" . $_SESSION['id'] . "'";
                $querymedell =$conexion->query($sqlmedell);
                $sqlbar= "SELECT nom_usu, id_usu FROM mq_usu WHERE (id_are != 10) AND id_are IN (0,4,6,7,11,12) AND id_reg = 4 AND usu_elim = 0 AND id_usu != '" . $_SESSION['id'] . "'";
                $querybar =$conexion->query($sqlbar);
                $sqlAna= "SELECT nom_usu, id_usu FROM mq_usu WHERE id_usu = 5186518 AND usu_elim = 0";
        ...
        <div class="col-md-2 col-sm-12">
            ...
            <select class="form-select" id="est_cot" name="est_cot" onchange="cotUsu(<?php echo date('m') . ',' . date('Y') ?>);">
        ...
        <div class="col-md-2 mb-3 mt-3">
            <form action="../cotizadorv3/reportes/reportePrueba.php" id="formExcel" method="POST" class="mt-2">
    ...
    <form role="form" action="../cotizadorv3/reportes/reporteAnual.php" id="formExcelAnual" method="POST">

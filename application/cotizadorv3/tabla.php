...
if (isset($_POST["id_usu"])) {
    if ($_POST["id_usu"] == "100" || $_POST["id_usu"] == "200" || $_POST["id_usu"] == "300") {
        ...
        if ($_POST["id_usu"] == "100") {
            $sql1 .= " AND us.grup_car IN(20) AND us.id_reg = 1 AND us.id_are = 7";
        } else if ($_POST["id_usu"] == "200") {
            $sql1 .= " AND us.grup_car IN(21,23,24,25)";
        } else {
            $sql1 .= " AND us.grup_car IN(22)";
        }
...
<div class="col-12 mt-3">
    <?php if ($query->rowCount() > 0) { ?>
        <div class="table-responsive">
            <table class="table table-bordered table-sm " id="tableCotizaciones" style="font-size: 0.6em;">
                ...
                <tbody>
                    <?php while ($r = $query->fetch(PDO::FETCH_ASSOC)) { ?>
                        <tr>
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-start" aria-labelledby="optionsDropdown">
                                        ...
                                        <a class="btn btn-link dropdown-item" href="<?php if ($r['id_tip_cot'] == 5  || $r['id_tip_cot'] == 6 || $r['id_tip_cot'] == 9) {
                                                                                        ...
                                                                                        echo "../cotizadorv3/cotizaciones.controller.php?id_coti=" . $r['id_coti'] . "&tip_cot=" . $r['id_tip_cot'] . "&id_cli=" . $r['id_cli'] . "&id=" . $r['id_usu'] . "&id_cont=" . $r['id_cont'];
                            ...
                            <td>
                                <div class="dropdown">
                                    <button type="button" class="btn btn-secondary btn-circle  <?php if ($r['est_cot'] == 0) {
                                                                                                    echo 'dropdown-btn';
                                                                                                }
                                                                                                if ($r["est_cot"] != 0) {
                                                                                                    echo 'btn';
                                                                                                    colorDropdown($r["est_cot"], $conexion);
                                                                                                } ?> btn-sm" id="btn-<?php echo $r['id_coti']; ?>" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false" <?php echo (isset($_POST['id_usu2'])) ? ' disabled' : ''; ?>>
                                        <i class="fa-regular fa-circle"></i>
                                    </button>
                                    <div class="dropdown-menu dropdown-menu-end" aria-labelledby="optionsDropdown">
                                        <?php foreach ($opc as $r12) { ?>
                                            <li>
                                                <a onclick="updEst(<?php echo $r12['id_est']; ?>,<?php echo $r['id_coti']; ?>,'<?php echo $r['nom_cont']; ?>','btn-<?php echo $r['id_coti']; ?>');" class="text-white btn btn<?php echo $r12['color']; ?> dropdown-item"><?php echo $r12['nom_est']; ?>

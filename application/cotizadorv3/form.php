...
if (isset($_POST['edit']) && $_POST['edit'] != '' && ($_POST['resp'] == 6 || $_POST['resp'] == 11)) {
	...
	$person = null;
	if ($query->rowCount() > 0) {
		while ($r = $query-> fetch(PDO::FETCH_LAZY)) {
			$person = $r;
			break;
		}
	}
...
<form role="form" id="form-cotizaciones">
		...
		<div class="col-md-4 col-sm-12 mb-3" id="remCiu1">
			...
			?>
				<select class="form-select" id="remCiu" name="remCiu" style="height: 35px;" required>
					<option value='<?php echo $rRemCiu1['id_ciu']; ?>'><?php echo $rRemCiu1['nom_ciu']; ?></option>
					<?php while ($rRemCiu = $queryRemCiu-> fetch(PDO::FETCH_ASSOC)) { ?>
						<option value='<?php echo $rRemCiu['id_ciu']; ?>'><?php echo $rRemCiu['nom_ciu']; ?></option>
					<?php } ?>
				</select>
			<?php } ?>
		</div>
	...
	<div class="row">
		...
		<div class="col-md-2 col-sm-6 mb-3">
			<input type="text" name="can_emp" id="can_emp" class="form-control" placeholder="C.U.E" readonly>
		</div>
	</div>
	<div class="row">
	...
	<div class="row">
		<div class="col-12 col-sm-12 mb-3" id="container-btnAgrega">
	...
	<div class="row" style="min-height: 250px;">
		<div class="col-12">
			<div class="table-responsive-sm">
				<div class="tab-final">
					<table id="table" class="table table-sm " style=" font-size: 100%;">
						...
						if (isset($_POST['edit']) && ($_POST['resp'] == 6 || $_POST['resp'] == 11)) {
							...
							while ($r = $query3-> fetch(PDO::FETCH_ASSOC)) { ?>
								<tr id="<?php echo $r['cod_pro']; ?>">
									...
									<td><?php echo $r['und_emp']; ?></td>
									<td><?php echo $r['can_emp']; ?></td>
									<td><?php echo $r['pre_cot']; ?></td>
									<td><?php echo $r['can_com']; ?></td>
									<td><?php echo $r['can_com'] * $r['pre_cot'] * $r['can_emp']; ?></td>
								</tr>
						<?php }
						} ?>
					</table>

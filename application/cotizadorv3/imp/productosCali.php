<?php 
	...
	$sql6="SELECT * from mq_usu, ind_cargos where id_usu = $cedAse and mq_usu.id_carg=ind_cargos.id_carg";
			...
			while($r=$query4->fetch(PDO::FETCH_ASSOC))
				...
				$html.='<br><p><strong>UNIDAD DE EMPAQUE:</strong> '.$r["und_emp"].' '.$r["can_emp"].'</p>';
			        	...
			        	if($r["sin_dev"]==1){
			        		...
			        		if (strlen($valorTotal)>6) {
			        			...
			        			$html.='<td style="background-color:transparent;" >$'.substr($valorTotal,0,-3).".".substr($valorTotal,-3).'</td>
			        			</tr>
			        			';
			        		}
				...
				$html.='
				        ...
				        if($datoscot['cot_iva']==1){
									$html.='<td colspan="2" align="center" style="background-color:transparent;">Total sin IVA</td>';
								}else{
									$html.='<td colspan="2" align="center" style="background-color:transparent;">Total</td>';
								}	
				      ...
				      if($datoscot['cot_iva']==1){
								$html.='<tr>
									<td colspan="2" style="border:none;background-color:transparent;"></td>
											...
											$html.='<td colspan="2" align="center" style="background-color:transparent;">$'.substr($totaliva,0,-6)."'".substr($totaliva,-6,-3).'.'.substr($totaliva,-3).'</td>
										</tr>';
											}else{
											$html.='<td colspan="2" align="center" style="background-color:transparent;">$'.substr($totaliva,0,-3).".".substr($totaliva,-3).'</td>
												</tr>
												';
											}
								}
...
<table style="font-size:15px;font-style:Arial;">
 ...
 if (isset($cotizador)) {
		...
		<tr>
			<td width="40%" style="background-color:transparent;">
				<p>
					...
					'.strtoupper($cotizador["nom_carg"]).' <br>
	...
	$sql7="SELECT * from mq_usu, ind_cargos where id_usu = $cedSac and mq_usu.id_carg=ind_cargos.id_carg";
	...
	';
		...
		if ($query7->rowCount()>0) {
			$sac=$query7->fetch(PDO::FETCH_ASSOC);
			$html2.='
			<table style="font-size:13px;font-style:Arial;background-color:transparent;">
			<tr>
				<td width="40%" style="background-color:transparent;">
					<p>
						<span style="font-weight:bold;">'.strtoupper($sac["nom_usu"]).'</span><br>

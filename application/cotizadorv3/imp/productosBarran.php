		...
		<br>
		 <table border="1"   autosize="1" style=" overflow: auto;">
			...
			while($r=$query4->fetch(PDO::FETCH_ASSOC))
				...
				//Terminado Aqui
				$html.='<tr>
			        		<td style="text-align:center; background-color:transparent;"><div><img style="max-width:100px;background-color:transparent;" src="../../documentos/cotizador/images/'.$r["img_pro"].'"></div><p>'.$r["cod_ref"].'</p></td>
				...
				$html.='<br><p><strong>UNIDAD DE EMPAQUE:</strong> '.$r["und_emp"].' '.$r["can_emp"].'</p>';
			        	...
			        	if($r["sin_dev"]==1){
			        		...
			        		if (strlen($valorTotal)>6) {
			        			$html.="<td style='background-color:transparent;' >$".substr($valorTotal,0,-6)."'".substr($valorTotal,-6,-3).'.'.substr($valorTotal,-3).'</td>
			        			</tr>
			        			';
			        		}else{
 ...
 <table style="font-size:14px;font-style:Arial;">
 	<tr><td width="3%" style="background-color:transparent;" >
 		...
 		<td style="line-height:1.5;background-color:transparent;">
 			Notas:<br>
 			 <ul>
				<li>*Si el pedido es inferior a $460.000 se cobra el valor de transporte: $8000 +IVA</li>
